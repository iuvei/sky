<template>
  <div class="registe">
    <div :class="[{'registe_head_pig':isfestival},'registe_head']">
    ...
    <div class="reg_con">
      <div class="reg_btn">
        <div @click="submitData"
             :class="[{'submmiting' : submmiting},{'submit_pig': isfestival}]">
          {{submmiting ? '注册中···' : '立即注册'}}
        </div>
      </div>
      <div class="con_one">
        <!-- <router-link to="/"> -->
        <span @click="openbigwin">在线客服</span>
        <!-- </router-link> -->
        <div @click="toLogin"
             class="to_login">
          <span>已有账号？直接登录</span>
      ...
      <div class="con_two">
        <span>
          1、账号必须是5-20位纯字母或数字且不可以0开头。
        </span>
        <span>
          2、登录密码必须是6-20位数字和字母的组合。
        </span>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    ...mapActions(["regGuestUser", "regUser", "getServiceUrl"]),
    /** 打开客服窗口 */
    async openbigwin() {
    ...
    checkRequest() {
      ...
      const rule = [
        {
          name: "input1",
          validator: /^[1-9a-zA-Z][0-9a-zA-Z]{4,20}$/,
          message: "请输入正确的用户账号!"
        },
        {
          name: "input2",
          validator: /^[0-9a-zA-Z]{6,20}$/,
          message: "请输入正确密码"
        },
        {
          name: "input4",
