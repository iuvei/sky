<template>
  <div class="accountInfo_main_body">
    ...
    <div class="content_word">
      ...
      <div class="hasContent" @click="toDetails(item)" v-show="this.allData!=0" v-for="(item, index) in allData" :key="index">
        <div class="left">
          <p>{{item.trad_type}}</p>
          <p>单号：{{item.dingdan}}</p>
        </div>
  ...
  data() {
    return {
      ...
      accountOptions: [
        ...
        {text: '提款退还', type: 6}, {text: '代理返点', type: 7}, {text: '中奖派送', type: 9},{text: '返水记录', type: 10},
        {text: '退款记录', type: 50}, {text: '投注记录', type: 51}, {text: '现金交易', type: 52}
      ],
      allData:[],
      isAllPresent: false,
      optionsIsShow: false,
  ...
  methods: {
    getData() {
      this.$ajax('request', {
        ...
        if(res.length == 0) {this.allIsShow = false}
        this.allData = res
