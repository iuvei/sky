<template>
  <div class="confirm_man_info">
    ...
    <p class="account_num"
       v-show="manData.man">
      <input type="text"
             :placeholder="'请输入'+text[1]"
             v-model="account">
    </p>
    <div class="man_info">
      <p v-if="!manData.man">
        <span>订单号：{{manData.dingdan}}</span>
        <button v-clipboard:copy="manData.dingdan"
      ...
      <p class="account"
         v-if="manData.man && manData.show">
        <span>收款账号：
          <i style="color:#fc7c34">{{manData.account}}</i>
        </span>
        <button v-clipboard:copy="manData.account"
                v-clipboard:success="onCopy"
                v-clipboard:error="onError">复制</button>
      </p>
      <p v-if="!manData.show">
      ...
      <div class="step"
           v-html="decode(stepTips[text[3]])"></div>
      <!-- <div v-html="str"></div> -->
