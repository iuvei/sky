  ...
  <div class="betRecordDetails_main_body">
    ...
    <div class="order_info">
      ...
      <div class="content"
        ...
        <!-- 0=待开奖, 1=已中奖 2=未中奖 3=撤单 4=追号 -->
        <p :class="[baseData.status==0?'wait':baseData.status==1?'win':baseData.status==2?'wait':baseData.status==3?'cancel':'default']">
          {{baseData.status==1?'已中奖':baseData.status==0?'待开奖':baseData.status==3?'已撤单':'未中奖'}}</p>
        <!-- <p>是</p> -->
        <p>{{baseData.wanfa}}</p>
      </div>
      <p class="title_word"
