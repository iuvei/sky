<template>
  <div class="papertemplateabc">
    <yd-accordion accordion>
      <yd-accordion-item class="paper_zd"
                         title="双面长龙排行"
                         open>
        <ul class="ranking">
          <li class="ranking_list">
            <div class='ranking_list_xq'
                 v-for="(item,index) in ba_dx_ds_hz_wdx.allKey"
                 :key="index">
      ...
      <yd-accordion-item class="paper_zd"
        ...
        <ul class="shortcut_button">
          <li class="shortcut_button_li"
              v-if="datalb.length"
              v-for="(item, index) in datalb"
              :key="index"
              :class="{'active': activeIndex===index}"
              @click="paper_road(index)">{{item.title}}</li>
        </ul>
      </yd-accordion-item>
    </yd-accordion>
...
export default {
  data() {
    return {
      ...
      data: [],
      pageid: 0,
      btnTitArrSsc: [...secondData.slice(0, 2), ...secondData.slice(4, 13)],
      btnTitArrEleven: [...secondData.slice(0, 13)],
      btnTitArrSand: [...secondData.slice(0, 2), ...secondData.slice(3, 9)],
      btnTitArrPk10: [...secondData.slice(19, 34)],
      btnTitArrPcdd: [...secondData.slice(34, 36)],
      btnTitArrK3: [...secondData.slice(0, 2), ...secondData.slice(13, 19)]
    };
  },
  computed: {
    ...mapState({
  ...
  methods: {
    ...
    _handleChangLongPaiHang(data) {
      ...
      const len = data.length > 20 ? 20 : data.length;
      for (let i = 0; i < len; i++) {
        // 先判断balls字段
        const ballsArr =
          data[i].balls && data[i].balls.length > 1
            ? data[i].balls.split("+")
            : [];
        const sumHZ = this._sumResults(ballsArr); // 总和值
        for (
          let j = this.js_tag === "pcdd" ? 3 : 0;
          j < (this.js_tag === "pk10" ? 5 : ballsArr.length);
          j++
        ) {
          const ball = ballsArr[j];
          // 记录每一位号码的大小单双状态。
          const dx = `${ball < ba_ov ? "小" : "大"}`; // 大小
          const ds = `${ball % 2 === 0 ? "双" : "单"}`; // 单双
          let lh = ""; // 龙虎
          if (this.js_tag === "pk10") {
                  ...
                  ba_dx_ds_hz_wdx[`lh_${j + 1}`] =
                    i === len - 1
                      ? `${titleArr[j]}-${lh}：${parseInt(
                        ba_dx_ds_hz_wdx[`lh_${j + 1}`].substr(1)
                      ) + 1}`
                      : `${lh}${parseInt(
                        ba_dx_ds_hz_wdx[`lh_${j + 1}`].substr(1)
                      ) + 1}`;
          ...
          if (
            ...
            const hzdx = `${sumHZ < baZH_ov ? "小" : "大"}`; // 和值大小
            const hzds = `${sumHZ % 2 === 0 ? "双" : "单"}`; // 和值单双
            let dxdsKeyArr = ["hzdx", "hzds"];
            let dxdsValueArr = [hzdx, hzds];
            if (this.js_tag === "11x5" && ballsArr.length === 5) {
              // 11x5,有尾大小，后二大小单双
              const hzwdx = `${
                parseInt(`${sumHZ}`.substr(-1)) >= 5 ? "尾大" : "尾小"
              }`; // 和值尾大尾小
              const hrhzInt = this._sumResults([ballsArr[3], ballsArr[4]]);
              const hrhzdx = hrhzInt < 12 ? "小" : "大"; // 后二和值大小； 小：3-11，大：12-21
              const hrhzds = hrhzInt % 2 === 0 ? "双" : "单"; // 后二和值单双
            ...
            for (let x = 0; x < dxdsKeyArr.length; x++) {
              const tit = dxdsKeyArr[x].includes("hr") ? "后二和" : "总和";
              if (ba_dx_ds_hz_wdx[dxdsKeyArr[x]] === null) {
                ...
                if (!ba_dx_ds_hz_wdx[dxdsKeyArr[x]].includes("：")) {
                  if (
                    ba_dx_ds_hz_wdx[dxdsKeyArr[x]].includes(dxdsValueArr[x])
                  ) {
                    ba_dx_ds_hz_wdx[dxdsKeyArr[x]] =
                      i === len - 1
                        ? `${tit}-${dxdsValueArr[x]}：${parseInt(
                          ba_dx_ds_hz_wdx[dxdsKeyArr[x]].substr(
                    ...
                    ba_dx_ds_hz_wdx[dxdsKeyArr[x]] = `${tit}-${ba_dx_ds_hz_wdx[
                      dxdsKeyArr[x]
                    ].substr(0, dxdsValueArr[x].length)}：${ba_dx_ds_hz_wdx[
                      dxdsKeyArr[x]
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const ballsArr =
          data[i].balls && data[i].balls.length > 1
            ? data[i].balls.split("+")
            : [];
        const sumHZ = this._sumResults(ballsArr);
        for (
          let j = this.js_tag === "pcdd" ? 3 : 0;
          j < (this.js_tag === "pk10" ? 5 : ballsArr.length);
          j++
          ...
          if (this.js_tag === "pk10") {
            const lh = `${ballsArr[j] > ballsArr[10 - j - 1] ? "龙" : "虎"}`;
            ...
            if (this.js_tag === "11x5") {
              ...
              if (ba123_hz_hr_wdx.hzwdx === null) {
                ...
                ba123_hz_hr_wdx.hrhz = [
                  parseInt(ballsArr[3]) + parseInt(ballsArr[4])
                ];
              } else {
    ...
    _scrollTableView() {
      // 切换清空上次
      const uls = document.querySelector(".value_form").querySelectorAll("ul");
      ...
      const titlist = this.datalb[this.activeIndex].title;
      console.log("titlist====", titlist);
      const tit = this.ba123_hz_hr_wdx[this.datalb[this.activeIndex].key]; // 被点击对应的数据
      const ba745_hz_hr_wdx = [];
      let ba_ov = 0,
        bahz_ov = 0;
