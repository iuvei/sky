...
export default {
  ...
  methods: {
    ...
    shake() {
      ...
      // 如果是时时彩，单独计算注数
      // if (
      //   (this.routeList.js_tag === 'ssc' && !this.isDoublePlay) ||
      //   this.routeList.js_tag === 'lhc'
      // ) {
      //   randomData.forEach(item => {
      //     item.data.filter(x => x.checked).forEach((x, i) => {
      //       if (this.filter && typeof this.filter === 'function') {
      //         this.filter(item, x, i)
      //       }
      //     })
      //   })
      // }
      ...
      this.playList.forEach(x => {
        x.submenu.forEach(y => {
          y.playlist.forEach((z, index) => {
            if (z.wanfa === wanfa) {
              const _index = this.playList
                .filter(c => c.showofficial === x.showofficial)
                .findIndex(d => d.menuid === x.menuid)
              this.playSwitch(x.showofficial ? '双面玩法' : '官方玩法')
              this.changeList(x, _index)
