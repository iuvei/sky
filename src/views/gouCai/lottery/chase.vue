<template>
  <div class="chase">
    <div class="chase_head">
      <span class="icon" @click="goBack"></span>
      <span class="title" v-if="tipShow">设置方案</span>
      <span v-if="!tipShow" class="chase_options">
        <i v-for="(item, index) in head_options" :key="index" :class="{choosed: headIndex == index}" @click="headIndex=index">{{item}}</i>
      </span>
      <span></span>
    </div>
    <count-down @activedData="getIssuNum"></count-down>
    <div class="tabs" v-show="!tipShow">
      <div class="ordinary">
        <div class="head">
          <!-- 智能追号 -->
          <span>追号<input type="number" v-model="qishu" placeholder="20" @change="changeQishu">期数</span>
          ...
          <!-- 翻倍追号 -->
          <span v-if="headIndex == 1">翻数<input type="number" v-model="fanshu" placeholder="1" @change="changeFanshu"></span>
          <!-- 平倍追号 -->
          <span v-if="headIndex != 2">起始倍数<input type="number" v-model="specialBeishu" placeholder="1" @change="createPlan(specialBeishu)"></span>
        </div>
        <div class="fanshu" v-if="headIndex==1">
          <span v-for="num in 3" :key="num" @click="changeFanshu(num)" :class="{'fanshuChoosed': fanshu == num}">{{num}}番</span>
        </div>
        <div :class="[{'need_arrow': headIndex==2},'change_plan']">
          <div class="plan_row">
            <span>注单选择:</span>
            <p @click="chooseZhushu = true">
              <span v-html="zhudan && zhudan.length && zhudan[0].formatedStr ? zhudan[0].formatedStr : '无'" class="cart_content"></span>
              <img :src="require('../../../img/daili/arrow_down.png')" alt="">
            </p>
            <span></span>
          </div>
          <span @click="popWinShow" v-if="headIndex == 2">修改方案</span>
        </div>
        <div class="title" style="height: auto;line-height: 1rem;">
        ...
        <div class="title animated" v-for="(item, index) in planData" :key="index">
          <span>{{index + 1}}</span>
          <span>{{(issueNum + index + '').slice(-4)}}</span>
          <p>
            <span @click='reduce(index)'>-</span>
            <input type="number" @blur="changeBeishu(item.beishu, index)" readonly="readonly" v-model="item.beishu" placeholder="1">
            <span @click='add(index)'>+</span>
          </p>
          <!-- <span>{{item.beishu}}</span> -->
          <span>{{(Math.round(item.jine*100)) / 100}}</span>
          <span style="text-align:center" v-yingli>
    ...
    <div class="bottom" v-show="!tipShow">
      <div class="left zhuihao_xiazhu">
        <yd-checkbox v-model="checkbox1" class="checkbox" color="#fff"></yd-checkbox>
        <div>
          <span>中奖后停止追号</span>
          <span>共
    ...
    <div class="plan_page" v-show="tipShow">
      <div class="panel_up">
        <div class="plan_row">
          <span>起始期号:</span>
          <p class="first_row" @click="openQishu">
            {{issueNum + qiShuIndex}}
            <img :src="require('../../../img/daili/arrow_down.png')" alt="">
          </p>
          <span>期</span>
        ...
        <div class="plan_row">
          ...
          <p>
            ...
            <input type="number" v-model="qishu" placeholder="10">
            <span @click='qishu += 1'>+</span>
          </p>
          <span>期</span>
        </div>
        <div class="plan_row">
          ...
          <p>
            ...
            <input type="number" v-model="beginBeiShu" placeholder="1">
            <span @click='beginBeiShu += 1'>+</span>
          </p>
          <span>倍</span>
        </div>
      ...
      <div class="expect_get">
        ...
        <div class="expect_get_content">
          <p>
            <span @click="chooseType1(firstVal)" :class="[{'checked': firstVal}, 'radio_option']"></span>
            <span>全程最低盈利率</span>
            <input type="number" :disabled="!firstVal" placeholder="30" v-model="planLowestPercent">
            <span>%</span>
          </p>
          <p>
            <span @click="chooseType2(secondVal)" :class="[{'checked': secondVal}, 'radio_option']"></span>
            <span>前</span>
            <input type="number" :disabled="!secondVal" placeholder="5" v-model="planBeforeQiShu">
            <span>期</span>
            <input type="number" :disabled="!secondVal" placeholder="50" v-model="planBeforeLowestPercent">
            <span>%</span>
          </p>
          <p>
            <span>之后盈利率</span>
            <input type="number" :disabled="!secondVal" placeholder="20" v-model="planAfterLowestPercent">
            <span>%</span>
          </p>
          <p>
            <span @click="chooseType3(thirdVal)" :class="[{'checked': thirdVal}, 'radio_option']"></span>
            <span>全程最低盈利</span>
            <input type="number" :disabled="!thirdVal" placeholder="30" v-model="planLowestPrice">
            <span>元</span>
          </p>
        </div>
      </div>
      <div class="create_button" @click="changePlan">
        生成追号
      </div>
    </div>
    <yd-popup v-model="chooseQishu" position="center" width="90%" class="chooseQishu_pop">
      <div class="wrap">
        <p v-for="(item, index) in 10" :key="item" :class="{'choosed': index==qiShuIndex}" @click="chooseIssueNum(index)">{{issueNum + index}}</p>
      </div>
    </yd-popup>
    <yd-popup v-model="chooseZhushu" position="center" width="90%" class="chooseZhushu_pop">
      <div class="wrap">
        ...
        <p v-for="(item, index) in cart" :key="index" @click="chooseZhu(item, index)" :class="{'choosed': zhudanIndex == index}">
          <span v-html="item.formatedStr ? item.formatedStr : ''"></span>
        </p>
      </div>
    </yd-popup>
  ...
  methods: {
    calculateBasicData(arr) {
      ...
      arr.forEach((item, index) => {
        let a = item.totalPrice / item.zhushu
        P += item.totalPrice
        N += item.zhushu

        let arr = item.peilvStr.replace(/\-/g, '1').split('|'),
          earn = 0
        arr.forEach((m, n) => {
    ...
    changeQishu() {
      let beishu = this.planData.length ? this.planData[0].beishu : 1
    ...
    getSubmitData() {
      ...
      this.zhudan.forEach((item, index) => {
