...
export default {
  ...
  computed: {
    ...
    nextQishu() {
      if (this.nexts && Array.isArray(this.nexts) && this.nexts.length) {
        ...
        console.log(
          'this.$store.state.betting.qishu',
          this.$store.state.betting.qishu,
          'newNextQishu',
          newNextQishu,
          'this.js_tag',
          this.$store.state.betting.js_tag
        )
  ...
  watch: {
    ...
    openless_leftTime(openless_leftTime) {
      const closeIsShow = openless_leftTime - (this.openless - this.stopless) > 0
  ...
  methods: {
      ...
      this.resTimer = setInterval(async () => {
        await this.getHistory(_this.routeLists.name_tag).then(() => {
          ...
          if (_this.balls.length) {
            if (this.routeLists.name_tag == 'xypknn') {
              if (this.isOne == 0) {
                this.isOne = 1
                return
              } else {
    ...
    // 随机号
    randomBall(min, max, length, type, hasPre) {  // 开奖球最小值  最大值  球长度   type1可重复 2不重复 type其他值为pk10专用 小于10十位是否加0
      const arr = []
