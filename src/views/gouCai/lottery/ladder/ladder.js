...
export default class {
  ...
  drawBg(reverse = false, fourRoot = true) {
    this.clear();
    this.start = 1;
    this.timer = '';
    this.ball_r = 14;
  ...
  setFlex() {
    const h =
      this.routes[this.routes.length - 1][1] -
      this.routes[0][1] +
      this.ball_r * 4;
    const canvasWidth = this.canvas.width;
    // this.ctx.save()
  ...
  drawBall_top_left(ballColor, txt) {
    ...
    ctx.font = this.font;
    ctx.fillText(
      txt || this.top_txt_left,
      ball_x_1,
      ball_y_1 - this.ball_r + this.txt_gap
    );
    ctx.closePath();
    ctx.restore();
  }
  ...
  drawBall_bottom_left(ballColor) {
    ...
    const ball_y_1 = this.routes[0][1];
    const ball_y_2 =
      ball_y_1 + this.routes[this.routes.length - 1][1] - this.routes[0][1];
    const ctx = this.ctx;
    ctx.beginPath();
    ctx.fillStyle = ballColor || this.ball_color_default;
    ...
    ctx.font = this.font;
    ctx.fillText(
      this.bottom_txt_left,
      ball_x_1,
      ball_y_2 + this.ball_r + this.txt_gap
    );
    ctx.closePath();
  }
  ...
  animate(callback) {
    ...
    if (start < points.length - 1) {
      ...
      ctx.beginPath();
      if ((this.fourRoot && this.reverse) || (!this.fourRoot && !this.reverse)) { this.drawBall_bottom_right(this.ball_color_active); } else this.drawBall_bottom_left(this.ball_color_active);
      this.ctx.closePath();
