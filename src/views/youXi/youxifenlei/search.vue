    ...
    <div class="yd-search">
      <div class="yd-search-input">
        <form action="#" class="search-input" @submit.prevent="submit">
          ...
          <yd-search-input type="search" :placeholder="placeholder" v-model="currentValue" :readonly="fullpage" @click.native="open" ref="search"></yd-search-input>
        </form>
      </div>
    </div>
    ...
    <template v-if="fullpage">
      <div class="yd-search yd-search-fly" :class="show ? 'yd-search-show' : ''" :style="{top: top}">
        <div class="yd-search-input">
          <form action="#" class="search-input" @submit.prevent="submit">
            <i class="search-icon"></i>
            <yd-search-input type="search" :placeholder="placeholder" v-model="currentValue" ref="search" :class="{'remove_x': currentValue != ''}"></yd-search-input>
          </form>
          <a href="javascript:;" class="cancel-text" @click="close(false)">{{cancelText}}</a>
        </div>
        <div class="yd-search-list-a" :style="{paddingBottom: top}">
          <div class="yd-search-list-item-a" v-for="(item, key) in result" @click="toOpenTry(item,tag)" :key="key" :class="{'yd-search-list-item-sw': userinfo.accountInfo.test === 2}">
            <img class="imgtp" :src="item.image_url" alt="图片">
        ...
        .yd-search-list-item-a {
            ...
            .triangle_try {
                ...
                .djan {
                    ...
                    border: solid poTorem(1px) #ff7c34;
                    display: flex;
                    justify-content: center;
                    align-items: center;
                    span {
