    ...
    <div class="tuijian_list">
      <yd-slider autoplay="3000">
        <yd-slider-item v-for="page in pages" :key="page">
          <div v-for="el in tjlist.slice((page-1)*pageSize,page*pageSize)" @click="toOpenTry(el,tag)"  :key="el" class="func_part">
            <div class="icon">
              <img :src="el.image_url" alt="图片">
            </div>
...
export default {
    ...
    computed: {
        ...
        pages() {
            let pages = Math.ceil(this.tjlist.length / this.pageSize)
            return pages
        }
    },
    mounted() {
