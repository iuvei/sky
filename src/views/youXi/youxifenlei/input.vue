<template>
  <div class="yd-input">
    ...
    <a href="javascript:;"
       class="yd-input-clear"
       tabindex="-1"
       @click="clearInput"
       v-show="showClearIcon && showClear && !isempty"></a>
    <span class="yd-input-error"
          v-if="showErrorIcon"
          v-show="(!!regex || !!min || !!max || required) && iserror && initError"></span>
    <span class="yd-input-warn"
          v-if="showRequiredIcon && showErrorIcon"
          v-show="(required || (!!min && min > 0)) && isempty && showWarn"></span>
    <span class="yd-input-success"
          v-if="showSuccessIcon"
          v-show="(!!regex || !!min || !!max || required) && !iserror && currentValue != ''"></span>
    <a href="javascript:;"
       v-if="type == 'password'"
       class="yd-input-password"
       :class="showPwd ? 'yd-input-password-open' : ''"
       tabindex="-1"
       @click.stop="showPwd = !showPwd"></a>
  </div>
</template>
...
export default {
  ...
  methods: {
    validatorInput(val, showError) {
      ...
      const reg = this.regexObj[this.regex]
        ? this.regexObj[this.regex]
        : this.trim(this.regex, "/");
