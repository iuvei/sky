    ...
    <yd-infinitescroll :callback="_callback" ref="infinitescrollDemo">
        <yd-list theme="1" slot="list">
            <ul class="left">
                ...
                <li v-for="(item, index) in list" v-if="index >=0" :key="index">{{(item.qishu+'').substr(-4)}}期</li>
            </ul>
            <ul class="left_number">
                <li class="title">
                    <span>平码</span>
                </li>
                <li v-for="(item,index) in list" :key="index" v-if="index >=0">
                    <span class="clearance" v-if="item.balls.length">
                    ...
                    <span v-if="!item.balls.length" style="color:#e33939;">
                        正在开奖
                    </span>
                </li>
            </ul>
            ...
            <div class="right">
                ...
                <ul class="content" v-for="(item,index) in list" :key="index" v-if="index >=0">
                    <li>{{item.balls?returnShengXiao(item.balls.split('+')[6]):'-'}}</li>
                    <li>{{item.balls?returnDanShuang(item.balls.split('+')[6]):'-'}}</li>
                    <li>{{item.balls?returnColor(item.balls.split('+')[6]):'-'}}</li>
                    <li>{{item.balls?returnAttribute(item.balls.split('+')[6]):'-'}}</li>
...
export default {
  ...
  methods: {
    ...
    returnAttribute(val) {
      if (goldArr.includes(val)) {
        return '金'
      } else if (woodArr.includes(val)) {
        return '木'
      } else if (waterArr.includes(val)) {
        return '水'
      } else if (theFireArr.includes(val)) {
        return '火'
      } else if (soilArr.includes(val)) {
        return '土'
      }
    },
    returnShengXiao(val) {
