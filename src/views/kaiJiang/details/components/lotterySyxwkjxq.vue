  ...
  <yd-infinitescroll :callback="_callback" ref="infinitescrollDemo">
    <yd-list theme="1" slot="list">
      <ul>
        ...
        <li v-for="(item, index) in list" v-if="index >= 0" :key="index">
...
export default {
    ...
    methods: {
        ...
        getRepeatTimes(item) {
            let index = this.list.findIndex(x => x.qishu === item.qishu) + 1
            if (index === this.list.length) return 0
            let next = this.list[index]
            let _balls_cur = item.balls.length
                    ? typeof item.balls === 'string'
                        ? item.balls.split('+')
                        : item.balls
                    : [], // 当前
                _balls_prev =
                    typeof next.balls === 'string'
                        ? next.balls.split('+')
                        : next.balls, // 当前的上1
                count = 0
