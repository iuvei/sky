          ...
          <yd-accordion-item :title="item.league_name"  class="group" v-for="item in data" :key="item.league_id">
            ...
            <ul>
              ...
              <li v-for="v in item.schedule" :key="v.history_id">
                <FootballTypeFmatchItem :data="v" class="group-v" :league="item.league_name" @clickCell="clickCell" :item="{ active}"></FootballTypeFmatchItem>
              </li>
            </ul>
          </yd-accordion-item>
        </yd-accordion>
...
import FootballTypeFmatchItem from "./components/FootballTypeFmatchItem";
export default {
  name: "FootballTypeFmatch",
  components: { FootballTypeFmatchItem },
  data() {
    return {
      data: 0,
      active: "",
