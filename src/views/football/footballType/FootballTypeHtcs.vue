          ...
          <yd-accordion-item :title="item.league_name" class="group" v-for="item in data" :key="item.league_id">
            ...
            <ul>
              ...
              <li v-for="v in item.schedule" :key="v.history_id">
                <FootballTypeWavesItem :data="v" class="group-v" :league="item.league_name" @clickCell="clickCell" :item="{ active}" :type="wavesType"></FootballTypeWavesItem>
              </li>
            </ul>
          </yd-accordion-item>
        </yd-accordion>
...
import FootballTypeWavesItem from './components/FootballTypeWavesItem'
export default {
  name: 'FootballTypeHtcs',
  components: { FootballTypeWavesItem },
  data() {
    return {
      data: 0,
      active: '',
      play_method: new Map([[1, 'TCS'], [2, 'HTCS']]),
      next_time: '',
      lockMark: false,
    }
  },
  computed: {
    ...mapState('football', ['timeCount', 'sport_id', 'playType']),
    wavesType() {
      return this.play_method.get(this.playType)
    },
  },
  methods: {
    ...mapActions('football', [
    ...
    clickCell(item) {
      ...
      let request = {
        ...
        k: item.k,
        p: item.p,
        play_method: this.play_method.get(this.playType),
        team: item.team,
