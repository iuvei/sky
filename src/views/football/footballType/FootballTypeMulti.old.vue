      ...
      <yd-infinitescroll :callback="loadMore" ref="loadMore_ref">
        ...
        <yd-accordion accordion class="yd-list" slot="list">
          ...
          <yd-accordion-item v-for="(league, league_index) in gameList" :key="league_index" :title="league.league_name">
            <div v-for="(game, game_index) in league.schedule" :key="game_index + '-' + game.schedule_id" class="one-game">
              ...
              <yd-flexbox class="one-table">
                ...
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 0, 0)" class="select-btn" @click.native="selectBet(game_index, game, 0, 0)">
                </yd-flexbox-item>
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 0, 1)" class="select-btn" @click.native="selectBet(game_index,game, 0, 1)">
                </yd-flexbox-item>
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 0, 2)" class="select-btn" @click.native="selectBet(game_index, game, 0 , 2)">
                </yd-flexbox-item>
                <div class="one-table-right">
                  <div class="border-bottom-no">
                    <yd-badge :class="!bet_obj[game_index + '-' + game.schedule_id] || !bet_obj[game_index + '-' + game.schedule_id].half ? 'bage-active' : 'bage-default'" @click.native="halfClick(game_index + '-' + game.schedule_id, false)">全场</yd-badge>
                  </div>
                </div>
              </yd-flexbox>
              ...
              <yd-flexbox class="one-table">
                ...
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 1, 0)" class="select-btn" @click.native="selectBet(game_index, game, 1, 0)">
                </yd-flexbox-item>
                <yd-flexbox-item class="select-btn"> &nbsp; </yd-flexbox-item>
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 1, 2)" class="select-btn" @click.native="selectBet(game_index, game, 1, 2)">
                </yd-flexbox-item>
                <div class="one-table-right">
                  <yd-badge :class="bet_obj[game_index + '-' + game.schedule_id] && bet_obj[game_index + '-' + game.schedule_id].half ? 'bage-active' : 'bage-default'" @click.native="halfClick(game_index + '-' + game.schedule_id, true)">半场</yd-badge>
                </div>
              </yd-flexbox>

              <yd-flexbox class="one-table">
                <div class="one-table-left">大小</div>
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 2, 3)" class="select-btn" @click.native="selectBet(game_index, game, 2, 3)">
                </yd-flexbox-item>
                <yd-flexbox-item class="select-btn"> &nbsp; </yd-flexbox-item>
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 2, 4)" class="select-btn" @click.native="selectBet(game_index, game, 2, 4)">
                </yd-flexbox-item>
                <div @click="allGame(game_index + '-' + game.schedule_id,game)" class="one-table-right">
              ...
              <yd-flexbox class="one-table">
                ...
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 3, 5)" class="select-btn" @click.native="selectBet(game_index, game, 3, 5)">
                </yd-flexbox-item>
                <yd-flexbox-item class="select-btn"> &nbsp; </yd-flexbox-item>
                <yd-flexbox-item v-one-html="renderHtml(game_index, game, 3, 6)" class="select-btn" @click.native="selectBet(game_index, game, 3, 6)">
                </yd-flexbox-item>
                <div @click="allGame(game_index + '-' + game.schedule_id,game)" class="one-table-right">
...
export default {
  ...
  watch: {
    ...
    bet_data(bet_data) {
      ...
      if (bet_data.length !== this.selectedKey.length) {
        bet_data.map(item => {
          let { is_all_method, play_method, schedule_id, team, k } = item
          let key =
        ...
        this.selectedKey.map((el, index) => {
          let flag = bet_data.some(item => el.includes(item.schedule_id))
          if (!flag) {
  ...
  methods: {
    ...
    renderHtml(game_index, game, type, l_type) {
      ...
      let l_field = this.l_fields[l_type]
      let data = game.bet_data || {}
      let html = ''
      let bet_txt = ''
      let select = true
      let schedule_id = game_index + '-' + game.schedule_id
      let half = this.bet_obj[schedule_id] && this.bet_obj[schedule_id].half
      if (half) field = 'H' + field
      let path = schedule_id + field + l_field
      ...
      switch (type) {
        ...
        case 3:
          data = data || []
          let index = data.findIndex(el => el.k === l_field)
          data = data[index]
      ...
      let selected = select && this.selectedKey.includes(path) && this.footer
    ...
    selectBet(game_index, { schedule_id, min_stake, max_stake }, type, l_type) {
      ...
      if (half) field = 'H' + field
      let path = schedule_id + field + l_field
      let data = this.bet_obj[path]
      // console.error(data);
      if (!data.select) return

      let isIndex = this.selectedKey.findIndex(el => el.includes(schedule_id))
