<template>
  <yd-flexbox class="win-bet">
    <yd-flexbox-item @click.native="betClick('WDL',hdata)"
                     :class="['is-betting',keys.includes(oneMatch.match_id + '||WDL||' + hdata.k)?'selected':'']">
      <h5>{{oneMatch.home_short_name}}</h5>
      <p>{{getItem(hdata.k).name+hdata.p}}</p>
    </yd-flexbox-item>

    <div @click="betClick('WDL',xdata)"
         :class="['vs is-betting',keys.includes(oneMatch.match_id + '||WDL||' + xdata.k)?'selected':'']">
      <h5> VS </h5>
      <p>{{getItem(xdata.k).name+xdata.p}}</p>
    </div>

    <yd-flexbox-item @click.native="betClick('WDL',vdata)"
                     :class="['is-betting',keys.includes(oneMatch.match_id + '||WDL||' + vdata.k)?'selected':'']">
