<template>
  <AppModel :show="show" @close="()=>{this.$emit('close')}">
    <ul class="group" v-show="show">
      <li v-html="renderStr" @click="userBalanceClick"></li>
      <li v-for="(item, key) in data" :key="key" :class="{active: key==active}" @click="switchs(key,item)">
...
export default {
  ...
  methods: {
    ...
    switchs(key, item) {
      ...
      switch (item.class) {
        case 'record':
          this.setBetRecordsType(5)
          this.$router.push({ name: 'goucaijilu' })
          break
        case 'rule':
          this.$router.push('/match/question')
          break
        default:
          break
      }
